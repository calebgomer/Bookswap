...
function sendEmail(arguments, callback) {
  ...
  var send = {
    ...
    message: {
      ...
      html: html || '(No Body)',
      text: text || '(No Body)',
      from_email: fromPerson.email || process.env.ROBOT_EMAIL,
      from_name: fromPerson.name || process.env.ROBOT_NAME,
      to: []
    }
  };
  if (replyTo) {
...
function sendPasswordReset(arguments, callback) {
  ...
  var emailText = util.format(passwordResetEmailText, name, process.env.ROOT_URL, resetToken, process.env.ROBOT_NAME);
  var emailHtml = util.format(passwordResetEmailHtml, name, process.env.ROOT_URL, resetToken, process.env.ROBOT_NAME);
